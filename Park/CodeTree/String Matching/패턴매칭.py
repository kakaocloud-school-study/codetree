...
n = len(a)
m = len(b)
# 매칭 가능 여부를 저장하는 배열을 초기화합니다.
is_pos = [[False for _ in range(m + 1)] for _ in range(n + 2)]

# 인덱스 접근을 편리하게 하기 위해 문자열 앞에 공백을 추가합니다.
a = " " + a
b = " " + b
...
# 문자열 a의 i번째와 표현식 b의 j번째까지가 서로 일치하면 true, 아니면 false
for j in range(m):
    for i in range(n + 1):
        if not is_pos[i][j]:
            continue
